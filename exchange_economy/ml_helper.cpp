...
Scenario::Scenario(
    ...
    n_persons(n_persons),
    goods_mean(goods_mean),
    goods_sd(goods_sd),
    utilFunc(util_params),
    steps_per_epoch(steps_per_epoch)
{
    ...
    // endowments are normally distributed
    endowments = goods_mean + torch::randn({n_persons, n_goods}) * goods_sd;
}


void Scenario::run_epoch_on_thread(int steps_per_epoch, torch::Tensor* loss) {
    // set up a new (identical) economy with each epoch
    auto economy = setup_economy(utilFunc, helper, endowments);
...
void Scenario::train(
    ...
    for (int i = 0; i < epochs; i++) {
        ...
        double loss_score = loss.item<double>() / static_cast<double>(threadcount * n_persons);
